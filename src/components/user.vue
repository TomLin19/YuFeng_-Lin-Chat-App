<template>
  <div class="user-info">
      ...
      <div class="user-nickname">
        ...
        <i @click="show = !show"></i>
          <div v-if="show" class="user-add-user">
            <span @click ="show_input =! show_input">Friend List</span>
            <input type="text" v-if="show_input" placeholder="User name" v-model="add_friend">
            <p class="user-add-btn" v-if="show_input" @click="addFriend">Add</p>
            <p class="user-add-btn" v-if="show_input" @click="deleteFriend">Delete</p>
          </div>
      </div>
    </div>
    ...
    <ul id="footer">
      ...
      <li class="footer-nav" 
          id="userlist" ß
          v-link="{path: '/user/userlist/talk/', activeClass: 'active', query: {username: 'Groupchat'}}"
          >
            ...
            if(result.friendslist.length) {
              result.friendslist.map((obj, index) => {
                if(obj.username == this.loginId) {
                  norepeatFriend = true
                  alert('Existing Friend',norepeatFriend)
                }
              })
            }
            if(!norepeatFriend) {
  ...
  .user-info {
    ...
    .user-name {
      ...
      .user-nickname {
        ...
        i {
          background: url(src/public/img/content.jpg) 0 -406px;
          width: 30px;
          height: 30px;
          vertical-align: middle;
          display: inline-block;
